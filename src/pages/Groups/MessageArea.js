...
const MessageArea = () => {
    ...
    const fetchData= async ({pageParam}) => {
        ...
        const data = await axios.get(
            `${process.env.REACT_APP_STORAGE_SERVICE_URL}/api/v1/messages/${groupControl.selectedGroup.groupId}/general/${offset}`
        );
        return {
            results: data.data.results,
            offset: offset + data.data.results.length + 1,
        };
    };
                            ...
                            <div className="name-msg">
                                {
                                    typeof msg === 'object' && msg !== null ?
                                        ...
                                        <ul key={index} ref={flattenedData.length === index + 1 ? lastElementRef : null}>
                                            <li>{msg}</li> <br />
                                        </ul>
                                }
                            </div>
